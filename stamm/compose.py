...
def _valid_addresses(value: str) -> bool:
    ...
    return bool(parsed) and all(address and "@" in address and not any(char in address for char in "\r\n") for _, address in parsed)
...
def parse_buffer(text: str) -> ComposeData:
    ...
    for index, line in enumerate(lines):
        ...
        if name == "Attach":
            ...
            values[name] = value
    return ComposeData(values["From"], values["To"], values["Cc"], values["Bcc"], values["Subject"], attachments, "\n".join(lines[split:]))


def format_buffer(data: ComposeData) -> str:
    lines = [
        f"From: {data.sender}", f"To: {data.to}", f"Cc: {data.cc}",
        f"Bcc: {data.bcc}", f"Subject: {data.subject}",
    ]
...
def validate(data: ComposeData) -> list[str]:
    ...
    for attachment in data.attachments:
        ...
        if not attachment.filename or Path(attachment.filename).name != attachment.filename or attachment.filename in (".", ".."):
...
def edit(config: Config, initial: ComposeData, errors: list[str] | None = None) -> ComposeData:
    ...
    while True:
        with tempfile.NamedTemporaryFile("w+", suffix=".eml", prefix="stamm-compose-", encoding="utf-8", delete=False) as stream:
...
def reply(message: Message, rendered_body: str, config: Config, *, all_recipients: bool = False) -> ComposeData:
    ...
    return ComposeData(sender, ", ".join(to), ", ".join(cc), "", _reply_subject(str(message.get("Subject", ""))), [], f"\nOn {date}, {author} wrote:\n{quote(rendered_body)}", message_id, references)
...
def forward(message: Message, rendered_body: str, config: Config) -> ComposeData:
    ...
    if not subject.lower().startswith("fwd:"):
        subject = "Fwd: " + subject
    return ComposeData(config.identities[0], subject=subject, body="\n---------- Forwarded message ----------\n" + header_block(message) + "\n\n" + rendered_body)

...
def build_message(data: ComposeData, config: Config, *, signing: bool = False, include_bcc: bool = True) -> EmailMessage:
    message = EmailMessage(policy=policy.default)
    message["From"] = data.sender
    for name, value in (("To", data.to), ("Cc", data.cc), ("Bcc", data.bcc if include_bcc else ""), ("Subject", data.subject)):
        if value:
            message[name] = value
    ...
    for attachment in data.attachments:
        ...
        message.add_attachment(attachment.path.read_bytes(), maintype=maintype, subtype=subtype, filename=attachment.filename)
    return message
...
def resume_draft(message: Message, workspace: Path) -> ComposeData:
    ...
    return ComposeData(
        str(message.get("From", "")), str(message.get("To", "")), str(message.get("Cc", "")),
        str(message.get("Bcc", "")), str(message.get("Subject", "")), attachments, body,
